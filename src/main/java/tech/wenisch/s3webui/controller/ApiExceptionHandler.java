...
public class ApiExceptionHandler {
    ...
    public ResponseEntity<Map<String, String>> handleAccessDenied(AccessDeniedException exception) {
        return ResponseEntity.status(403).body(Map.of("message", resolveMessage(exception)));
    }

    @ExceptionHandler(IllegalArgumentException.class)
    public ResponseEntity<Map<String, String>> handleIllegalArgument(IllegalArgumentException exception) {
        return ResponseEntity.badRequest().body(Map.of("message", resolveMessage(exception)));
    }

    @ExceptionHandler(SecretDecryptionException.class)
    public ResponseEntity<Map<String, String>> handleSecretDecryption(SecretDecryptionException exception) {
        return ResponseEntity.status(500).body(Map.of("message", resolveMessage(exception)));
    }
